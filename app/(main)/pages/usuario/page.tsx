...
const Crud = () => {
    ...
    const saveUsuario = () => {
        ...
        if (!usuario.login.trim()) {
            ...
            return; // Impede que o código continue se o login estiver vazio
        }
        ...
        if (!usuario.id || usuario.id === 0){
            console.log("Usuário sendo enviado:", JSON.stringify(usuario, null, 2));

            const usuarioParaSalvar = { ...usuario };
            if (usuarioParaSalvar.id === 0){
                delete usuarioParaSalvar.id;
            }

            usuarioService.inserir(usuarioParaSalvar)
                .then((response) => {
                    setUsuarioDialog(false);
                    setUsuario(usuarioVazio);
                    setUsuarios([]);
